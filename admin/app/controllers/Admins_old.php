...
class Admins extends CI_Controller {
	...
	public function get_admin_list()
	{
		...
		*/
		...
		if($this->input->is_ajax_request())
		{
			...
			$result_list = $this->Admin_model->get_admin_list($sWhere, $sOrder, $sLimit);
			...
			foreach($rResult as $aRow)
			{
				...
				$actions .= "<br>";
				$actions .= "</div>";
				$row[] = json_encode(array("sl_no" => $i++, "fname" =>$aRow["firstname"], "lname" => $aRow["lastname"], "email" => $aRow["email_id"], "contact" => $aRow["contact_no"], "status" => $status_html, "actions" => $actions, "id" => $id));
				$row[] = "";
				$row[] = "";
				$row[] = "";
				$row[] = "";
	...
	public function add_admin()
	{
		if($this->input->is_ajax_request() && count($this->input->post()) > 0)
		{
			...
			$this->form_validation->set_rules('employee_id', 'Employee Id', 'required|trim|is_unique[admin.employee_id]');
				...
				else
				{
					...
					try
					{
						if($admin = $this->Admin_model->new_admin($admin_data))
						{
							...
							{
								$mail_sent = $this->Sendmail_model->send_admin_register($data);
								if($mail_sent === true)
								{
									$this->db->trans_commit();
									$response["msg"] = "Admin added successfully.";
	...
	public function update_pwd()
	{
		...
		if($this->input->is_ajax_request() && count($this->input->post()) > 0)
		{
			...
			if($this->form_validation->run() !== false)
			{
				...
				if(is_numeric($id) && $id > 0)
				{
					$response["title"] = "Admin Management - Change Password";
