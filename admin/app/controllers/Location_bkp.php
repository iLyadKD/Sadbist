...
class Location extends CI_Controller {
		...
		*/
		...
		if($this->input->is_ajax_request())
		{
			...
			foreach($rResult as $aRow)
			{
				...
				$actions .= "<a class='btn btn-danger btn-xs has-tooltip delete_country' data-placement='top' title='Delete' href='javascript:void(0);'><i class='icon-remove'></i></a>\n";
				$actions .= "<br>";
				$actions .= "</div>";
	...
	public function regions_list()
	{
		...
		* you want to insert a non-database field (for example a counter or static image)
		*/
		$aColumns = array("", "country_en", "region", "name_en");
		...
		if($this->input->is_ajax_request())
		{
			...
			foreach($rResult as $aRow)
			{
				...
				$actions .= "<br>";
				$actions .= "</div>";
				$row[] = json_encode(array("sl_no" => $i++, "country" =>$aRow["country"], "country_name" => $aRow["country_en"], "region" => $aRow["region"], "region_name" => $aRow["name_en"], "status_html" => $status_html, "actions" => $actions, "id" => $id));
				$row[] = "";
				$row[] = "";
				$row[] = "";
				$row[] = "";
	...
	public function cities_add()
	{
		if($this->input->is_ajax_request() && count($this->input->post()) > 0)
		{
			...
			$this->form_validation->set_rules("state_name", "Region Name", "trim|required");
			$this->form_validation->set_rules("city", "City", "trim|required");
			$this->form_validation->set_rules("latitude", "Latitude", "trim|required");
			$this->form_validation->set_rules("longitude", "Logitude", "trim|required");
			...
			else
			{
				...
				$keyword = $city.", ".$region_name.", ".$country_name;
				$keyword = str_replace(", ,", ",", $keyword);
				$city_data = array("country" => $country, "region" => $region, "city" => $city, "latitude" => $latitude, "longitude" => $longitude, "keyword" => $keyword);
				...
				if($is_exists === false)
				{
					...
					$response["msg"] = "City name \"".$city."\" is already exists.";
			}
			echo json_encode($response);exit;
		}
		else
		...
		*/
		...
		if($this->input->is_ajax_request())
		{
			...
			foreach($rResult as $aRow)
			{
				...
				$actions .= "<a class='btn btn-danger btn-xs has-tooltip delete_airport' data-placement='top' title='Delete' href='javascript:void(0);'><i class='icon-remove'></i></a>\n";
				$actions .= "<br>";
				$actions .= "</div>";
	...
	public function airports_add()
	{
		if($this->input->is_ajax_request() && count($this->input->post()) > 0)
		{
			...
			$this->form_validation->set_rules("airport_en", "Airport Name", "trim|required");
			$this->form_validation->set_rules("airport_fa", "Airport Name", "trim|required");
		...
		*/
		...
		if($this->input->is_ajax_request())
		{
			...
			foreach($rResult as $aRow)
			{
				...
				$actions .= "<a class='btn btn-danger btn-xs has-tooltip delete_airline' data-placement='top' title='Delete' href='javascript:void(0);'><i class='icon-remove'></i></a>\n";
				$actions .= "<br>";
				$actions .= "</div>";
