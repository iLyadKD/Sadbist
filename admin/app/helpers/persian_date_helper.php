<?php 
	...
	function to_date($g_date,$input)
	{
		...
		$persian_date=gregorian_to_persian($g_year,$g_month,$g_day);
		if($input=='Y') return $persian_date[0];
		if($input=='y') return substr($persian_date[0],-2);
		if($input=='M') return persian_month_names()[$persian_date[1]];
		if($input=='m') return $persian_date[1];
		if($input=='D') return persian_day_names()[date('w')];
		...
		if($input=='compelete') 
		{
			$persian_d=$persian_date[2];
			if($persian_d{0}=='0') $persian_d=substr($persian_d,1);
				return persian_day_names()[date('w')].' '.$persian_d.' '.persian_month_names()[$persian_date[1]].' '.$persian_date[0]; 
		}
	}
	
	function date_fa($input)
	{
		return to_date(date('Y').date('m').date('d'),$input);		
	}
	...
	function date_to($j_date)
	{
		...
		$gregorian_date=persian_to_gregorian($j_year,$j_month,$j_day);
