...
class General {
	...
	{
		$query = "SELECT `id`, `country_".$lang."` AS `country_name` FROM `countries` WHERE `status` = '1' ORDER BY CASE WHEN `id` = '".OTHER_COUNTRY."' THEN 1 ELSE 0 END, `country_".$lang."`";
	...
	{
		$query = "SELECT `title_".$lang."` AS `title`,`content_".$lang."` AS `content` FROM `pages` WHERE `status` = '1'";
		$result = $this->ci->db->query($query);
	...
	public function get_regions($country_code, $lang = DEFAULT_LANG)
	{
		$query = "SELECT `country`, `region`, `name_".$lang."` AS `name` FROM `regions` WHERE `country` = '".$country_code."' ORDER BY CASE WHEN `region` = '".NO_REGION."' THEN 1 ELSE 0 END, `region`";
		$result = $this->ci->db->query($query);
		return $result->num_rows() > 0 ? $result->result() : false;
	}

	/* Get complete city list for particular region and country */
	public function get_cities($region_code, $country_code, $lang = DEFAULT_LANG)
	{
		if($region_code === NO_REGION)
			$query = "SELECT `id`, `city_".$lang."` AS `city` FROM `cities` WHERE `country` = '".$country_code."' ORDER BY `city_".$lang."` AS ";
		else
			$query = "SELECT `id`, `city_".$lang."` AS `city` FROM `cities` WHERE `region` = '".$region_code."' AND `country` = '".$country_code."' ORDER BY `city_".$lang."` AS ";
		$result = $this->ci->db->query($query);
