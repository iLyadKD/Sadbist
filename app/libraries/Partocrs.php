...
class Partocrs {
	...
	public function authenticate($api_data)
	{
		if($api_data->status === "2")
		{
			...
			$password = $this->ci->encrypt->decode(base64_decode($api_data->live_pwd));
			$office_id = $api_data->live_extra;
			$url = $api_data->live_url;
		}
		elseif($api_data->status === "1")
		{
			$username = $api_data->test_user;
			$password = $this->ci->encrypt->decode(base64_decode($api_data->test_pwd));
			$office_id = $api_data->test_extra;
			$url = $api_data->test_url;
		}
		else
			return $result = array("status" => false, "result" => $this->ci->lang->line("invalid_api_credentials"));
		...
		$result = array("status" => false, "result" => $this->ci->lang->line("invalid_api_response"));
		if($response !== false)
		{
	...
	public function low_fare_air_search($api_data, $search_data)
	{
		...
		if($response === false)
		{
			...
			$travel_preference = "";
			if(@$srch_dtls->preference_airport !== null && !empty(@$srch_dtls->preference_airport))
			{
				$travel_preference = "<d2p1:VendorPreferenceCodes xmlns:d3p1='http://schemas.microsoft.com/2003/10/Serialization/Arrays'>";
				$travel_preference .= "<d3p1:string>".$srch_dtls->preference_airport."</d3p1:string>";
				$travel_preference .= "</d2p1:VendorPreferenceCodes>";
			}
			$xml = "<AirLowFareSearch xmlns:i='http://www.w3.org/2001/XMLSchema-instance' xmlns='http://schemas.datacontract.org/2004/07/ServiceModel.Request'>
						...
						<RequestOption>All</RequestOption>
			...
			$req_file = FCPATH."app/cache/partocrs/search/request/".$api_data->id.$hash.".xml";
	...
	public function air_rules($api_data, $search_data, $fare_source_code)
	{
		...
		if($response !== false)
		{
			...
			$status = $response->Success;
			if($status === true)
				$result = array("status" => true, "result" => json_encode($response->FareRules), "type" => TYPE_JSON);
			else
			{
				$search_file = FCPATH."app/cache/partocrs/search/".$api_data->id.$hash.".json";
				if(file_exists($search_file))
					unlink($search_file);
				$result = array("status" => false, "result" => $response->Error->Message, "type" => TYPE_STRING);
			}
		}
		return $result;
	}
	...
	public function air_revalidate($api_data, $search_data, $flight_details, $cached = "0")
	{
		...
		if($response !== false)
		{
			...
			$status = $response->Success;
			if($status === true)
				$result = array("status" => "true", "result" => "Success", "redirect" => "$redirect", "type" => TYPE_STRING);
			else
			{
				$search_file = FCPATH."app/cache/partocrs/search/".$api_data->id.$hash.".json";
				if(file_exists($search_file))
					unlink($search_file);
				$error_msg = $this->ci->lang->line("no_err_msg_to_display");
				if(!is_null($response->Error))
					$error_msg = $response->Error->Message;
				$result = array("status" => "false", "result" => $error_msg, "redirect" => "true", "type" => TYPE_STRING);
			}
		}
		return $result;
	}
	...
	public function air_baggages($api_data, $search_data, $fare_source_code)
	{
		...
		if($response !== false)
		{
			...
			$status = $response->Success;
			if($status === true)
				$result = array("status" => true, "result" => json_encode($response->BaggageInfoes), "type" => TYPE_JSON);
			else
			{
				$search_file = FCPATH."app/cache/partocrs/search/".$api_data->id.$hash.".json";
				if(file_exists($search_file))
				...
				if(!is_null($response->Error))
					$error_msg = $response->Error->Message;
				$result = array("status" => false, "result" => $error_msg, "type" => TYPE_STRING);
			}
		}
		return $result;
	}
	...
	public function air_book($api_data, $search_data, $flight_details, $details, $reserve = true)
	{
		...
		$response = false;
		if(file_exists($search_file))
			return array("status" => false, "result" => $this->ci->lang->line("resubmit_book_error"));
		if($reserve === true and $flight_details->fare_type === "WebFare")
			return array("status" => false, "result" => $this->ci->lang->line("reserve_not_possible"));
		if($response === false)
		{
			if($api_data->status === "2")
				$url = $api_data->live_url;
			...
			for($i = 0; $i < $adult_cnt; $i++)
			{
				...
				$extra_service = isset($details->adult_baggage[$i]) && !empty($details->adult_baggage[$i]) ? "<d2p1:ExtraServiceId xmlns:d5p1='http://schemas.microsoft.com/2003/10/Serialization/Arrays'>
												<d5p1:int>".$details->adult_baggage[$i]."</d5p1:int>
											</d2p1:ExtraServiceId>" : "";
				$travellers_details .= "<d2p1:AirTraveler>
											...
											".$national_id."
											<d2p1:Nationality>".$details->adult_nationality[$i]."</d2p1:Nationality>
											<d2p1:PassengerName>
			...
			for($i = 0; $i < $child_cnt; $i++)
			{
				...
				$extra_service = isset($details->child_baggage[$i]) && !empty($details->child_baggage[$i]) ? "<d2p1:ExtraServiceId xmlns:d5p1='http://schemas.microsoft.com/2003/10/Serialization/Arrays'>
												<d5p1:int>".$details->child_baggage[$i]."</d5p1:int>
											</d2p1:ExtraServiceId>" : "";
				$travellers_details .= "<d2p1:AirTraveler>
											...
											".$national_id."
											<d2p1:Nationality>".$details->child_nationality[$i]."</d2p1:Nationality>
											<d2p1:PassengerName>
			...
			for($i = 0; $i < $infant_cnt; $i++)
			{
				...
				$extra_service = isset($details->infant_baggage[$i]) && !empty($details->infant_baggage[$i]) ? "<d2p1:ExtraServiceId xmlns:d5p1='http://schemas.microsoft.com/2003/10/Serialization/Arrays'>
												<d5p1:int>".$details->infant_baggage[$i]."</d5p1:int>
											</d2p1:ExtraServiceId>" : "";
				$travellers_details .= "<d2p1:AirTraveler>
											...
											".$national_id."
											<d2p1:Nationality>".$details->infant_nationality[$i]."</d2p1:Nationality>
											<d2p1:PassengerName>
			...
			$contact_details = "<d2p1:Email>".$details->email."</d2p1:Email>
								<d2p1:PhoneNumber>".$details->contact."</d2p1:PhoneNumber>";
			$xml = "<AirBook xmlns:i='http://www.w3.org/2001/XMLSchema-instance' xmlns='http://schemas.datacontract.org/2004/07/ServiceModel.Request'>
		...
		if($response !== false)
		{
			...
			if($status === true)
			{
				...
				if($flight_details->fare_type === "WebFare")
					$confirm_stat = true;
				$result = array("status" => true, "result" => json_encode(array("ticket_id" => $response->UniqueId, "deadline" => $response->TktTimeLimit, "reserve" => true, "confirm" => $confirm_stat)));
	...
	public function air_cancel($api_data, $ticket_id)
	{
		...
		$response = false;
		if(file_exists($search_file))
			return json_encode(array("status" => false, "result" => $this->ci->lang->line("already_ticket_cancelled")));
		if($api_data->status === "2")
			$url = $api_data->live_url;
		elseif($api_data->status === "1")
			$url = $api_data->test_url;
		...
		if($response !== false)
		{
			...
			else
			{
				...
				if(!is_null($response->Error))
					$error_msg = $response->Error->Message;
				$result = json_encode(array("status" => false, "result" => $error_msg));
			}
		}
		return $result;
	}
	...
	public function air_confirm($api_data, $ticket_id)
	{
		...
		$request_file = FCPATH."app/cache/partocrs/confirm/request/".$api_data->id.$ticket_id.".xml";
		$error_file = FCPATH."app/cache/partocrs/confirm/error/".$api_data->id.$ticket_id.".json";
		$response = false;
		if(file_exists($search_file))
			return json_encode(array("status" => false, "result" => $this->ci->lang->line("ticket_already_issued")));
		if($api_data->status === "2")
			$url = $api_data->live_url;
		elseif($api_data->status === "1")
			$url = $api_data->test_url;
		...
		if($temp_res->Success === true)
			...
			file_put_contents($error_file, $response);
		$result = false;
		if($response !== false)
		{
			$response = json_decode($response);
	...
	public function air_message_queue($api_data, $status)
	{
		if($api_data->status === "2")
			$url = $api_data->live_url;
		elseif($api_data->status === "1")
		...
		$book_status = array(
							...
							"11" => "Pending",
							"12" => "Waitlist",
							"20" => "TicketinProcess",					//can set schedule
							"21" => "Ticketed",							//can set schedule
							"22" => "TicketedChanged",					//can set schedule
							"23" => "TicketedScheduleChange",			//can set schedule
							"30" => "Cancelled",
							"40" => "Urgent",
							"41" => "ApiUrgent",
							"42" => "TktTimeLimitChange"
							);

		$xml = "<AirMessageQueue xmlns:i='http://www.w3.org/2001/XMLSchema-instance' xmlns='http://schemas.datacontract.org/2004/07/ServiceModel.Request'>
					<SessionId>".$session."</SessionId>
					<Status>".$book_status[$status]."</Status>
				</AirMessageQueue>";
		$response = $this->curl($url."Air/AirMessageQueue", $xml);
		$result = false;
		if($response !== false)
		{
			$response = json_decode($response);
			$status = $response->Success;
			if($status === true)
				$result = json_encode(array("status" => true, "result" => $response->Messages, "type" => TYPE_JSON));
			else
			{
				$error_msg = $this->ci->lang->line("no_err_msg_to_display");
				if(!is_null($response->Error))
					$error_msg = $response->Error->Message;
				$result = json_encode(array("status" => false, "result" => $error_msg, "type" => TYPE_STRING));
			}
		}
		return $result;
	}

	public function air_remove_message_queue_item($api_data, $status, $ids = array())
	{
		if($api_data->status === "2")
			$url = $api_data->live_url;
		elseif($api_data->status === "1")
		...
		$items = "";
		foreach ($ids as $id)
			$items .= "<d2p1:AirRemoveMessageQueueItem>
							<d2p1:Status>".$book_status[$status]."</d2p1:Status>
							<d2p1:UniqueId>".$id."</d2p1:UniqueId>
						</d2p1:AirRemoveMessageQueueItem>";
		$xml = "<AirRemoveMessageQueue xmlns:i='http://www.w3.org/2001/XMLSchema-instance' xmlns='http://schemas.datacontract.org/2004/07/ServiceModel.Request'>
					<Items xmlns:d2p1='http://schemas.datacontract.org/2004/07/ServiceModel.Entities'>
						".$items."
					</Items>
					<SessionId>".$session."</SessionId>
				</AirRemoveMessageQueue>";
		$response = $this->curl($url."Air/AirRemoveMessageQueue", $xml);
		$result = false;
		if($response !== false)
		{
			$response = json_decode($response);
			$status = $response->Success;
			if($status === true)
				$result = json_encode(array("status" => true, "result" => "Success", "type" => TYPE_STRING));
			else
			{
				$error_msg = $this->ci->lang->line("no_err_msg_to_display");
				if(!is_null($response->Error))
	...
	public function air_trip_details($api_data, $ticket_id)
	{
		$search_file = FCPATH."app/cache/partocrs/trip_data/".$api_data->id.$ticket_id.".json";
		$response = false;
		if(file_exists($search_file))
			if(filemtime($search_file) > strtotime("-2 minutes", time()))
				$response = file_get_contents($search_file);
		if($response === false)
		{
			...
			$xml = "<AirTripDetails xmlns:i='http://www.w3.org/2001/XMLSchema-instance' xmlns='http://schemas.datacontract.org/2004/07/ServiceModel.Request'>
							<SessionId>".$session."</SessionId>
							<UniqueId>".$ticket_id."</UniqueId>
						</AirTripDetails>";
			$response = $this->curl($url."Air/AirTripDetails", $xml);
			$temp_res = json_decode($response);
			if($temp_res->Success === true)
				file_put_contents($search_file, $response);
		}
		...
		if($response !== false)
		{
			...
			$status = $response->Success;
			if($status === true)
				$result = array("status" => true, "result" => json_encode($response), "type" => TYPE_JSON);
			else
			{
				$error_msg = $this->ci->lang->line("no_err_msg_to_display");
				if(!is_null($response->Error))
	...
	public function parse_flights($api_data, $search_data, $itineraries)
	{
		...
		foreach ($itineraries as $flight_idx => $flight_obj)
		{
			...
			if(in_array($flight_type, array(ONEWAY, ROUNDTRIP)))
			{
				...
				{
					if($temp_res["stops"] < (count($route_return) - 1))
						$temp_res["stops"] = count($route_return) - 1;
			...
			foreach ($route_departure as $dept_idx => $dept_obj)
			{
				...
				$temp_dept["operating_airline_name"] = isset($airlines[$dept_obj->OperatingAirline->Code]) ? $airlines[$dept_obj->OperatingAirline->Code]->airline_name : $dept_obj->OperatingAirline->Code;
				...
				$temp_res["duration"] = date("H:i", strtotime($temp_res["duration"]) + strtotime($dept_obj->JourneyDuration));
				...
				if($flight_type === MULTICITY)
				{
					...
					{
						if(!isset($temp_res["stops"]) || $temp_res["stops"] < $openjaw_stops)
							$temp_res["stops"] = $openjaw_stops;
						$openjaw_stops = 0;
						$dpt_idx++;
						$temp_res["origin_destination"][] = array(
			...
			foreach ($route_return as $arrv_idx => $arrv_obj)
			{
				...
				$temp_arrv["operating_airline_name"] = isset($airlines[$arrv_obj->OperatingAirline->Code]->airline_name) ? $airlines[$arrv_obj->OperatingAirline->Code]->airline_name : $arrv_obj->OperatingAirline->Code;
				...
				$temp_res["duration"] = date("H:i", strtotime($temp_res["duration"]) + strtotime($arrv_obj->JourneyDuration));
