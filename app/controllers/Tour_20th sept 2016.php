...
class Tour extends CI_Controller {
	...
	public function search() {
		...
		$location_info['keyword_from'] = $get_data['keyword_from'];
		$location_info['keyword_to'] = $get_data['keyword_to'];
        $data['location_info'] = json_encode($location_info);
		$langauge =$this->session->userdata('default_language');
		$data['title'] ="Tours ";
       // $data['type'] = $this->Home_model->get_package_types();
		$data['neighbours'] = $this->Home_model->get_neighbours($this->data['default_language']);
		$data['arr_city'] = json_encode($arr_city);
		//pr($data['neighbours']);exit;
	...
	public function result() {
		...
		$this->session->set_userdata('tours_count',ceil($data['tours_count']/10));
	...
	public function details($id,$hotel_id,$total_tour_price,$transportation_cost,$trans,$tour_date,$master_id,$currency,$rating,$base_price,$duration,$location_info,$short_info){
		//pr(base64_decode($location_info));exit;
		...
		$tour_date = base64_decode($tour_date);
		$master_id = base64_decode($master_id);
		$total_tour_price = base64_decode($total_tour_price);
		$transportation_cost = base64_decode($transportation_cost);
		$base_price = base64_decode($base_price);
		$trans = explode("-",base64_decode($trans));
		$duration = base64_decode($duration);
		$trans_flag = $trans[0];
		$trans_id = $trans[1];
		$data['short_info'] = base64_decode($short_info);
		
		//pr($check_other_rooms);
		...
		if($this->session->userdata['keyword_from'] != '') {
			$from_city = explode(",",$this->session->userdata['keyword_from']);
		}
		else {
			$from_city = '';
			redirect(base_url());
		...
		if($this->session->userdata['keyword_to'] != '')
			$to_city = explode(",",$this->session->userdata['keyword_to']);
		else
			$to_city = '';	
			
			
			
		$data['from_city'] = $from_city[0];
		$data['to_city'] = $to_city[0];
		$data['tour_date'] = date('dS M Y',strtotime($tour_date));
		$data['tour_count'] = $this->session->userdata['tour_count'];
	...
	function check_tour(){
		if($this->input->is_ajax_request())
		{
			...
			$selective_tours = $this->Tour_model->check_tour($from_id);
			
			$city_ids = array();
			if(!empty($selective_tours)){
				foreach($selective_tours as $cities){
					$city_ids[] =  $cities['d_city'];
				}
			}
			//pr($city_ids);exit;
			echo json_encode($city_ids);exit;
		}
	}
	...
	function prebook_submit(){
		if($this->input->is_ajax_request() === true && count($this->input->post()) > 0){
			...
			$final_tourists = array_merge($tourists_spcl,$tourists);
			...
			if(!empty($child['twotosix']['salutation'])){
				...
				if(!empty($twotosix)){
					$final_tourists = array_merge($twotosix,$final_tourists);
				}
			}
			...
			if(!empty($child['sixtotwelve']['salutation'])){
				...
				if(!empty($sixtotwelve)){
					$final_tourists = array_merge($sixtotwelve,$final_tourists);
				}
			}
			...
			if(!empty($child['twelvetosixteenth']['salutation'])){
				...
				if(!empty($twelvetosixteenth)){
					$final_tourists = array_merge($twelvetosixteenth,$final_tourists);
				}
			}
			...
			if(!empty($child['infants']['salutation'])){
				...
				if(!empty($infants)){
					$final_tourists = array_merge($infants,$final_tourists);
				}
			}
			...
			if($save_booking_id){
				...
				/*if($save_booking_details == 1){
					$reference_no = $this->get_reference_no($save_booking_id);
					$voucher['reference_no'] = $reference_no;
					$voucher['travellers'] = $final_tourists;
					$voucher['departures'] = $departures;
					$voucher['summery'] = $after_confirm;
					$this->load->view('tour/voucher',$voucher);
				}*/
