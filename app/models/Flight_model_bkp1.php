...
class Flight_Model extends CI_Model {
	...
	{
		$query = "SELECT IF(`created` > '".date("Y-m-d H:i:s", strtotime("-15 minutes"))."', 'false', 'true') AS `expired` FROM `search_temp` WHERE `results` = '1' AND `id` = '$id'";
		$result = $this->db->query($query);
	...
	function get_rflight_result($id, $filter)
	{
		$query = "SELECT count(*) AS `total`, max(`total_cost`) AS `max`, min(`total_cost`) AS `min`, GROUP_CONCAT(DISTINCT `airline`) AS `airlines`, GROUP_CONCAT(DISTINCT `airport`) AS `airports`, GROUP_CONCAT(DISTINCT `stops`) AS `stops`, GROUP_CONCAT(DISTINCT `currency`) AS `currency`, IF(`valid_status` > '".date("Y-m-d H:i:s", strtotime("-15 minutes"))."', 'false', 'true') AS `expired` FROM `flight_search_result` WHERE `search_id` = '$id' AND `results` = '1'";
		// $query .= " AND `airline` <> 'IR'";
		//price 
		if(isset($filter["price_filter"]) && !empty($filter["price_filter"]))
		{
	...
	function available_iflights($id, $filter, $lang = DEFAULT_LANG)
	{
	...
	function available_rflights($id, $filter, $lang = DEFAULT_LANG)
	{
	...
	public function get_airports($ids = array(), $lang = DEFAULT_LANG)
	{
		...
		$query = "SELECT `airport_code`, `airport_$lang` AS `airport`, `city_code`, `city_$lang` AS `city`, `country_$lang` AS `country`, `country` AS `country_code`, `city_link`, `airport_link` FROM `flight_airports`, `countries` ".$where." GROUP BY `airport_code` ORDER BY `city_code`, `airport_code`";
		$result = $this->db->query($query);
		return $result->num_rows() > 0 ? $result->result() : false;
	}
