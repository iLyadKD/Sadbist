...
class Flight_Model extends CI_Model {
	...
	{
		$query = "SELECT `markups`.* FROM `markups` INNER JOIN `markup_types` ON `markup_types`.`id` = `mt_id` AND `markup_priority` = '0' AND (`markup_types`.`user_type` IS NULL OR `markup_types`.`user_type` = '".B2C_USER."') AND (`api` IS NULL OR `api` = '$id') ORDER BY `user_type` DESC LIMIT 0,1";
		$result = $this->db->query($query);
		return $result->num_rows() > 0 ? $result->row() : false;
	}
	...
	function set_booking($id, $data)
	{
		$set = "";
		foreach ($data as $key => $value)
			$set .= "`$key` = '$value', ";
		$set = substr_replace($set, "", -2);
		$query = "UPDATE `bookings` AS `bo`, `booking_details` AS `bd` SET $set WHERE `bo`.`id` = `bd`.`id` AND `bo`.`id` = '$id'";
		$this->db->query($query);
		return $this->db->affected_rows() > 0 ? true : false;
	}

	function clear_old_flight_search($api_type = FLIGHT)
	{
		$old_srch_time = date("Y-m-d H:i:s", strtotime("-1 hours", strtotime(date("Y-m-d H:i:s"))));
		$query = "DELETE FROM `search_temp` WHERE `api_type` = '$api_type' AND `created` < '$old_srch_time'";
		$this->db->query($query);
	...
	{
		$query = "SELECT IF(`valid_status` > '".date("Y-m-d H:i:s", strtotime("-15 minutes"))."', 'false', 'true') AS `expired` FROM `flight_search_result` WHERE `search_id` = '$id'";
		$result = $this->db->query($query);
	...
	function get_flight_result($id, $filter)
	{
		$query = "SELECT count(*) AS `total`, max(`total_cost`) AS `max`, min(`total_cost`) AS `min`, GROUP_CONCAT(DISTINCT `airline`) AS `airlines`, GROUP_CONCAT(DISTINCT `airport`) AS `airports`, GROUP_CONCAT(DISTINCT `stop_out`) AS `stops`, GROUP_CONCAT(DISTINCT `currency`) AS `currency`, IF(`valid_status` > '".date("Y-m-d H:i:s", strtotime("-15 minutes"))."', 'false', 'true') AS `expired` FROM `flight_search_result` WHERE `search_id` = '$id' AND `airline` <> 'IR'";
		//price 
		...
		if(isset($filter["stops"]))
		{
			...
			if($stops !== "")
				$query .= " AND (`stop_out` IN ($stops) OR `stop_in` IN ($stops))";
		}
		...
		if(isset($filter["departure_filter"]) && !empty($filter["departure_filter"]))
		{
			...
			if(!empty($range))
			{
				...
				$query .= " AND (`departure_tm` BETWEEN '$min' AND '$max')";
			}
		}
		...
		if(isset($filter["return_filter"]) && !empty($filter["return_filter"]))
		{
			...
			if(!empty($range))
			{
				$min = $this->general->minutes_to_hours(trim($range[0]));
				$max = $this->general->minutes_to_hours(trim($range[1]));
				$query .= " AND (`arrival_tm` BETWEEN '$min' AND '$max')";
			}
		}
	...
	function available_flights($id, $filter, $lang = DEFAULT_LANG)
	{
		$query = "SELECT `flight_search_result`.*, `airline_$lang` AS `airline_name`, if(`stop_out` = '0', 'Direct Stop Coach', if(`stop_out` = '1', 'One Stop Coach', 'Multi Stop Coach')) AS `stops_text` FROM `flight_search_result` INNER JOIN `flight_airlines` ON `airline` = `airline_code` WHERE `search_id` = '$id' AND `airline` <> 'IR'";
		//price 
		if(isset($filter["price_filter"]) && !empty($filter["price_filter"]))
		{
			$range = explode("-", $filter["price_filter"]);
	...
	public function get_airlines($search_id, $ids, $lang = DEFAULT_LANG)
	{
		...
		$query = "SELECT `airline_code`, `airline_$lang` AS `airline_name`, GROUP_CONCAT(CONCAT(`stop_out`, ':', `total_cost`)) AS `total_price` FROM `flight_airlines`, (SELECT * FROM(SELECT `airline`, `total_cost`, `stop_out` FROM `flight_search_result` WHERE `search_id` = '$search_id' ORDER BY `total_cost`) AS `a` GROUP BY `airline`, `stop_out`) AS `flight_search_result` WHERE `airline` = `airline_code` AND `airline_code` IN (".$id_list .") GROUP BY `airline`";
		if(!empty($id_list))
		{
			$result = $this->db->query($query);
			return $result->num_rows() > 0 ? $result->result() : false;
	...
	public function get_airports($ids = array(), $lang = DEFAULT_LANG)
	{
		...
		$query = "SELECT `airport_code`, `airport_$lang` AS `airport`, `city_code`, `city_$lang` AS `city`, `country_$lang` AS `country`, `city_link`, `airport_link` FROM `flight_airports`, `countries` ".$where." GROUP BY `airport_code` ORDER BY `city_code`, `airport_code`";
		$result = $this->db->query($query);
		return $result->num_rows() > 0 ? $result->result() : false;
	}
