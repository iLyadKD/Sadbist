...
var fEDate = EngDate_to_persian_EngDigit(fdate.getDate(), m + 12, y);
var cur_lang = document.getElementsByTagName("head")[0].getAttribute("data-language");

regionals_data = {// Available regional settings, indexed by language/country code
            'en': { // Default regional settings - English/US
                renderer: {}, // this.defaultRenderer
                changelangText: 'Change Calendar',
                changelangStatus: 'Change Calendar',
                language: cur_lang,
                prevText: '&lt;',
                prevStatus: 'Previous',
                prevJumpText: '&lt;&lt;',
                ...
                nextText: '&gt;',
                nextStatus: 'View next month',
                nextJumpText: '&gt;&gt;',
                nextJumpStatus: 'View next year',
                currentText: 'Current',
                currentStatus: 'View this Month',
                todayText: 'Today',
                todayStatus: 'Today Date',
                clearText: 'Clear',
                clearStatus: 'Clear date',
                closeText: 'Close',
                closeStatus: 'Close datepicker',
                yearStatus: 'Change Year',
                monthStatus: 'Change Month',
                weekText: 'Wk',
                weekStatus: 'Week',
            ...
            'fa': { // Default regional settings - Persian
                renderer: {}, // this.defaultRenderer
                language: cur_lang,
                changelangText: 'تغییر تقویم',
                changelangStatus: 'تغییر تقویم',
                prevText: 'قبلی',
                prevStatus: 'نمایش ماه قبل',
                prevJumpText: '&lt;&lt;',
                prevJumpStatus: 'نمایش به سال گذشته',
                nextText: 'بعدی',
                nextStatus: 'نمایش ماه آینده',
                nextJumpText: '&gt;&gt;',
                nextJumpStatus: 'نمایش در سال آینده',
                currentText: 'کنونی',
                currentStatus: 'نمایش ماه جاری',
                todayText: 'امروز',
                todayStatus: 'ماه نمایش امروز',
                clearText: 'پاک کردن',
                clearStatus: 'پاک کردن تمام تاریخ',
                closeText: 'بستن',
                closeStatus: 'بستن datepicker',
                yearStatus: 'تغییر سال',
                monthStatus: 'تغییر ماه',
                weekText: 'Wk',
                weekStatus: 'هفته سال',
                dayStatus: 'انتخاب کنید DD, M d, yyyy',
                defaultStatus: 'تاریخ انتخاب',
                isRTL: true
            }
        }
var regional_data = cur_lang === "fa" ? regionals_data.fa : regionals_data.en;
...
(function ($) { // Hide scope, no $ conflict
    ...
    $.JQPlugin.createPlugin({
        ...
        defaultRenderer: {
			...
			'<div class="calendars-nav">{link:prev}{link:next}</div>{months}' +
			'{popup:start}<div class="calendars-ctrl">{button:today}{button:changelang}{button:close}</div>{popup:end}' +
        ...
        commands: {
            changelang: {
                text: 'changelangText', status: 'changelangStatus', // Previous month
                keystroke: { keyCode: 32, ctrlKey: true }, //  Ctrl + Space
                ...
                action: function (inst)
                {
                    var c;
                    var lang = "fa";
                    if (inst.options.calendar === $.calendars.instance("fa"))
                        lang = "en";
                    c = $.calendars.instance(lang);
                    cl = $.calendars.instance(cur_lang);
                    c.local.monthNames = cl.local.monthNames;
                    c.local.monthNamesShort = cl.local.monthNamesShort;
                    c.local.dayNames = cl.local.dayNames;
                    c.local.dayNamesShort = cl.local.dayNamesShort;
                    c.local.dayNamesMin = cl.local.dayNamesMin;
                    $(this).datepick('option', { calendar: c});
                    $(document).find(".is-datepick").val("").datepick('option', { calendar: c});
                    // $(this).datepick('option', { calendar: c, isRTL: inst.options.calendar.local.isRTL, language: lang, changelangText : inst.options.calendar.local.changelangText, changelangStatus : inst.options.calendar.local.changelangStatus, todayText : inst.options.calendar.local.todayText, todayStatus : inst.options.calendar.local.todayStatus, closeText : inst.options.calendar.local.closeText, closeStatus : inst.options.calendar.local.closeStatus, prevText : inst.options.calendar.local.prevText, prevStatus : inst.options.calendar.local.prevStatus, nextText : inst.options.calendar.local.nextText, nextStatus : inst.options.calendar.local.nextStatus});
                    // $(document).find(".is-datepick").val("").datepick('option', { calendar: c, isRTL: inst.options.calendar.local.isRTL, changelangText : inst.options.calendar.local.changelangText, changelangStatus : inst.options.calendar.local.changelangStatus, todayText : inst.options.calendar.local.todayText, todayStatus : inst.options.calendar.local.todayStatus, closeText : inst.options.calendar.local.closeText, closeStatus : inst.options.calendar.local.closeStatus, prevText : inst.options.calendar.local.prevText, prevStatus : inst.options.calendar.local.prevStatus, nextText : inst.options.calendar.local.nextText, nextStatus : inst.options.calendar.local.nextStatus});
        ...
        _generateContent: function (elem, inst) {
            ...
            if (cur_lang === "fa")
                $(".calendars").css("font-family","WYekan");
            else
                $(".calendars").css("font-family","sans-serif");
            return picker;
        },
        ...
        _generateMonth: function (elem, inst, year, month, calendar, renderer, first) {
            ...
            monthHeader = (first ? calendar.formatDate(monthHeader, calendar.newDate(year, month, calendar.minDay)) :
				calendar.formatDate(monthHeader, calendar.newDate(year, month, calendar.minDay)));
            var yr = monthHeader.split(' ')[1];
            var mn = monthHeader.split(' ')[0];
            mn = getPerMonthDigit(mn);
            ...
            for (var week = 0; week < numWeeks; week++) {
                ...
                for (var day = 0; day < calendar.daysInWeek() ; day++) {
                    ...
                    days += this._prepare(renderer.day, inst).replace(/\{day\}/g,
						...
						dateInfo.content || GetPersianDay(drawDate.day()) : '&nbsp;') +
						(selectable ? '</a>' : '</span>'));
                    drawDate.add(1, 'd');
                    jd++;
                }
            ...
            // monthHeader = '<span class="persian-year-2">' + monthHeader.split(' ')[0] + '</span>' + '<span class="persian-year-1">' + monthHeader.split(' ')[1] + '</span>';

            //monthHeader += '<br/>'+ '<span class="english-year-1">'  + EngMonthName + '</span>' + '<span class="english-year-2">' +' ' + EngYear + '</span>';

            //var EngweekHeader = this._prepare(renderer.weekHeader, inst).
            //  replace(/\{days\}/g, this._generateEngDayHeaders(inst, calendar, renderer));
        ...
        _generateDayHeaders: function (inst, calendar, renderer) {
            ...
            for (var day = 0; day < calendar.daysInWeek() ; day++) {
                ...
                header += this._prepare(renderer.dayHeader, inst).replace(/\{day\}/g,
					...
					calendar.local.dayNames[dow] + '">' + '<span class="persian-day-name-1">' + calendar.local.dayNamesMin[dow] + '</span>' + '</span>');
            }
...
function GetEngDay(dd, mm, yy) {
    var Nodays;

    Nodays = NoDayInPersianMonth(mm, yy);
    var dt = persianDate_to_Eng(dd, mm, yy);
    if (dd == "01" || dd == "1") {
        stmn = dt.split(' ')[1];
        EngYear = dt.split(' ')[2];
...
function PerDayNameToEngDayName(nm) {
    prMonName = "";
...
function NoDayInPersianMonth(Mon, year) {
    var days;
    var y;
